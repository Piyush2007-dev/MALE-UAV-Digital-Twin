...
from constants import (
    CYLINDERS, FIRE_CRANK, CYCLE_DEG,
    TAU_EGT, TAU_CHT, CHT_WARMUP_T, FAULT_TAU,
    BASELINE_EGT, BASELINE_CHT, MAX_OP, BASELINE_MAP,
    BASELINE_FF
)
...
def get_telemetry(altitude: float = 10000, throttle: float = 100.0, fault_mode: str = "normal"):
    ...
    # Ease rate derived from time delta (dt) so fault progression is 
    # independent of the polling frequency.
    fault_ease_rate = 1.0 - math.exp(-dt / FAULT_TAU)

    # Compute baseline expectations given current atmospheric and thermal conditions
    ...
    if fault_mode == "misfire":
        ...
        egt_target = cht_target = kurt_target = op_target = rpm_target = map_target = 0.0

    # Fault boosts use a time-scaled easing function to smoothly ramp up
    # anomalies over realistic physical timeframes rather than immediate steps.
    ...
    # ── Residuals ─────────────────────────────────────────────────────────
    # Cross-cylinder thermal spreads help the ML model isolate cylinder imbalance (e.g. misfires)
    ...
    # ── ML anomaly score ──────────────────────────────────────────────────
    # The anomaly detector evaluates the multi-dimensional residual vector
    ...
    # ── Wear & RUL ───────────────────────────────────────────────────────
    # Accumulate simulated engine wear scaled to actual time delta (dt)
    wear_rate = {
        "normal":  1.0,
        "misfire": 15.0,
        "cooling": 10.0,
    ...
    try:
        ...
        rul_hours = rul_effective_time_remaining / max(wear_rate, 0.001)
    except ValueError:
        rul_hours = 0.0
